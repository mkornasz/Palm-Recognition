...
namespace PalmRecognizer
{
    class PalmTool
    {
        ...
        {
            CannyParamHigh = cannyHigh;
            CannyParamLow = cannyLow;
            ContrastParam = contrast;
            BrightnessParam = brightness;
            _palmOriginal = new Mat(palmFilename, Emgu.CV.CvEnum.LoadImageType.Color);
            _newImage = _palmOriginal.ToImage<Bgr, Byte>();
            _palmOriginalImage = _palmOriginal.ToImage<Bgr, Byte>();
            _palmBw = new Mat();
        ...
        {
            _newImage = _palmOriginalImage.Convert(b => SaturateCast(ContrastParam * b + BrightnessParam));
        ...
        {
            _palmEdges = new Mat();
            CvInvoke.Canny(matToFindEdges, _palmEdges, CannyParamLow, CannyParamHigh);
        }
